$(document).ready(function() {
	...
	var tokenHash=$("input[name=csrf_token]").val(); //console.log(tokenHash)
			...
			callback: function(confirmed) {
				if (confirmed) {
					...
					$.ajax({
						...
						data : 'submit=submit&id=' + JSON.stringify(selected_files) + '&csrf_test_name=' + tokenHash,
						dataType : 'JSON',
	...
	$('#list-file-container').delegate('.btn-delete-file', 'click', function(e) {
		...
		$.ajax({
			type : 'post',
			url : filepicker_server_url + 'ajaxDeleteFile',
			data : 'submit=submit&id=' + get_id_file($li) + '&csrf_test_name=' + tokenHash,
			dataType : 'JSON',
			success : function(data) {
				if (data.status == 'error') {
					show_alert('Error !!!', data.message, 'error');
			...
			callback: function(confirmed) {
				if (confirmed) {
					$.ajax({
						...
						data: 'submit=submit&ajax=ajax'+'&csrf_token=' + tokenHash,
						dataType: 'json',
