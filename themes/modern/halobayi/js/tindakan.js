...
jQuery(document).ready(function () {
  ...
  $(document).on("click", ".btn.gantiDokter", function () {
    id = $(this).attr("data-id");
    $bootbox = bootbox.dialog({
      title: '<i class="fa-solid fa-user-doctor me-1"></i> Ganti Dokter',
      message:
        '<div class="text-center text-secondary"><div class="spinner-border"></div></div>',
      buttons: {
        cancel: {
        ...
        success: {
          label: "Simpan",
          className: "btn-warning submitGantiDokter",
          callback: function () {
            $bootbox.find(".alert").remove();
            ...
            $.ajax({
              type: "POST",
              url: base_url + "tindakandokter/updateGantiDokter",
              data: new FormData(form),
              processData: false,
              contentType: false,
              dataType: "json",
    ...
    $.get(
      base_url + "tindakandokter/ajaxKlikGantiDokter?no_reg=" + id,
      function (html) {
        $button.prop("disabled", false);
        $bootbox.find(".modal-body").empty().append(html);
      },
